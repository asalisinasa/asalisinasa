...
- CSS Modules for styling; terminal-style design tokens in `src/styles/tokens.css`.
...
- **Sections** are self-contained React Server Components under `src/app/(site)/_sections/*`, each with its own CSS Module. The `Terminal` shell (`src/app/(site)/_components/terminal/`) wraps them in a terminal-style card.
