...
The reason this matters for AI-assisted decisions: an assistant defaults to agreeable, optimistic responses. If you ask "is this a good plan?" it will find reasons to say yes. The premortem breaks this pattern by forcing the frame into "this is dead, explain how it died." The agent stops looking for reasons your plan will work and starts explaining how it fell apart.
...
- `AGENTS.md` or equivalent (business context, preferences, constraints)
...
- **This is not a council-style perspective review.** A council gives multiple perspectives on a decision right now. The premortem sends the agent into the future where the decision already failed and works backward to explain why. Different psychological mechanism, different output. If the user seems to want multiple perspectives rather than failure analysis, offer the council approach instead.
